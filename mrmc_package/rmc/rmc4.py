...
class RMC4:
    ...
    def table_init(self):
        ...
        if self.r2chi == 'k':
            ...
            self.r_factor_i = np.array([self.exp[_].r_factor_cross(
                self.table[_].chi *
                np.transpose([self.table[_].ft[self.exp[_].r_range]])) for _ in range(self.exp.size)])
        ...
        while trials > 0:
            ...
            if not tau == 1:
                if self.r2chi == 'k':
                    ...
                    r_factor_i = np.array([self.exp[_].r_factor_cross(
                        self.table[_].chi *
                        np.transpose([self.table[_].ft[self.exp[_].r_range]])) for _ in range(self.exp.size)])
